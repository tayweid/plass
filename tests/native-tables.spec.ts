...
test('native table keystrokes map numbering and defer contextual geometry past first paint', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    const { eqKey } = await import('/src/equations.ts');
    const labelsBefore = eqKey.getState(window.view.state)?.labels;
    ...
    window.__tableHotPathProbe = {
      ...
      finish() {
        ...
        const afterGeometry = geometry
          ...
          .map((entry) => entry.kind);
        return {
          labelsReused: eqKey.getState(window.view.state)?.labels === labelsBefore,
...
test('default native table uses the intrinsic centered Typst box model', async ({ page }) => {
  ...
  const metrics = await page.evaluate(async () => {
    ...
    const [{ compileDocProofSvg }, { parseTypstSvg }] = await Promise.all([
      import('/src/pdf.ts'),
      import('/src/safe-svg.ts'),
    ]);
    const svg = await compileDocProofSvg(window.view.state.doc);
    if (!svg) throw new Error('Typst did not return default-table proof geometry');
    // Use the product's sanitizer/parser: Typst's prefixed HTML text layer is
    // deliberately not XML-parseable, while the rendered SVG path is.
    ...
    const compiledWidthPt = Math.max(...horizontalRules.map((rule) => rule.length));
    const compiledHeightPt =
      Math.max(...horizontalRules.map((rule) => rule.localY)) -
      Math.min(...horizontalRules.map((rule) => rule.localY));
    if (!Number.isFinite(compiledWidthPt) || !Number.isFinite(compiledHeightPt)) {
...
test('contextual controls mutate native structure and retain rich content', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    const { CellSelection } = await import('/src/table-editor.ts');
    const { state } = window.view;
    const cellPositions: number[] = [];
    state.doc.descendants((node, pos) => {
      const role = node.type.spec.tableRole;
      if (role === 'cell' || role === 'header_cell') cellPositions.push(pos);
      return cellPositions.length < 2;
    });
    window.view.dispatch(state.tr.setSelection(new CellSelection(
