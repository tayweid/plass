...
export function demoDoc(): PMNode {
  return schema.nodes.doc.create({ bib: { name: 'references.bib', content: DEMO_BIB } }, [
    ...
    p(
      ...
      ' — the same optimizing paragraph breaker used by TeX and Typst — and imposed on ordinary, fully editable browser text. Click anywhere and start typing: the paragraph re-typesets around your cursor in about a millisecond. Toggle ',
      { text: 'TeX layout', marks: ['em'] },
      ' in the toolbar to compare against the browser’s greedy line breaking; watch the ragged interword spacing and the rivers of white space come and go.',
    ...
    p(
      'Insert figures from the toolbar, or just paste or drop an image. Click a figure to select it, click the label chip to name it, and reference it in text the same way as equations: ',
      { ref: 'fig:sd' },
    ...
    p(
      ...
      { text: '⊞', marks: ['code'] },
      '); Tab moves between cells, and row/column controls appear in the toolbar while you are inside one. Tables export as native Typst ',
