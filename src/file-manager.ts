...
export class FileManager {
  ...
  async loadHandle(handle: FileSystemFileHandle) {
    ...
    this.hooks.message(
      warnings.length
        ? `Opened ${file.name} — ${warnings.length} block(s) preserved as raw Typst`
        : `Opened ${file.name}`,
    );
    try {
  ...
  async restoreLast(): Promise<boolean> {
    ...
    try {
      const handle = (await idbGet('last')) as FileSystemFileHandle | null;
      if (!handle) return false;
      const perm = (await handle.queryPermission?.({ mode: 'readwrite' })) ?? 'denied';
  ...
  async openRecent(entry: RecentEntry) {
    try {
      let perm = (await entry.handle.queryPermission?.({ mode: 'readwrite' })) ?? 'granted';
      if (perm !== 'granted') {
        perm = (await entry.handle.requestPermission?.({ mode: 'readwrite' })) ?? 'denied';
      }
      if (perm !== 'granted') {
...
async function addRecent(handle: FileSystemFileHandle, fileName: string) {
  try {
    ...
    kept.unshift({ name: fileName, time: Date.now(), handle });
